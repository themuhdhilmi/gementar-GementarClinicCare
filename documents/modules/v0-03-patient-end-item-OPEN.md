...
| **PAT-OPEN-17** | **The MyKad parser exists twice.** `identity.ts` on the server is the authority and has 15 tests; `patients/new/page.tsx` has a deliberate copy so the fields fill while the receptionist is still looking at the card, rather than after a round trip. The copy is simpler — no century warning for a card that could be either — and **the web workspace has no test runner**, so nothing stops the two drifting. They agree today; nothing would say if they stopped. | Either a shared package the two import, or vitest in `apps/web` with the same table of cases against both. The second is cheaper and catches drift, which is the actual risk. |
