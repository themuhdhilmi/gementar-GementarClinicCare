...
| # | Item | Done when |
|---|---|---|
| **RX-OPEN-02** | ~~**No printout**~~ **— the printout exists (`v0-13-documents.md`).** `POST /prescriptions/:id/print` renders a numbered, letterheaded, signed prescription, refuses a draft, and prints the patient's full identity number only when something on it is controlled. **The controlled-drug fields are still unanswered (RX-F-08, RX-F-18, RX-Q-03):** there is no quantity in words, and nothing has been checked against what an inspector actually expects. | Ask the clinic what their inspector wants on the page (`RX-Q-03`). Adding fields to a template that already exists is minutes; guessing at them was always the expensive part. |
| ~~**RX-OPEN-03**~~ | ~~**No stock figure beside a medicine (RX-F-04).**~~ **Closed 2026-09-21.** The stock ledger was built for procedures; a product search carries on-hand at the caller's branch with a `stockKnown` flag so "none" and "not known" stay distinguishable; the prescribing panel shows both the figure and the nearest expiry on every result; and an item the shelf cannot cover carries an amber `OUT_OF_STOCK` warning that informs without blocking. | Done. |
...
| # | Item | Lands with |
|---|---|---|
| ~~**RX-OPEN-07**~~ | ~~**Nothing can be dispensed.**~~ **Closed 2026-09-21.** `DSP` dispenses against the prescription, moves each item to `DISPENSED`, `PARTIAL` or `DECLINED`, and the prescription settles to `COMPLETED` on its own. | Done. |
| ~~**RX-OPEN-08**~~ | ~~**`ENC-OPEN-15` stays open on purpose.**~~ **Closed 2026-09-21.** `DSP` registered the guard at the same moment it gained the ability to satisfy it, which is what this item was waiting for. | Done. |
