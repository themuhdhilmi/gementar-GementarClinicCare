...
| # | Item | Lands with |
|---|---|---|
| **BIL-OPEN-09** | **Nothing can be printed (BIL-F-19).** No invoice, no receipt, no "COPY" stamp on a reprint. The data is complete. | `v0-13-documents.md`, which is Phase 2 and still unstarted — the same gap that leaves consultations, prescriptions and immunisation records unprintable. |
| **BIL-OPEN-10** | **Nothing collects money.** An issued invoice carries its balance and stays `ISSUED`. `amount_paid`, `rounding_adjustment` and the `PARTIAL`/`PAID` transitions are the columns the immutability trigger deliberately leaves writable. Cash rounding to 5 sen is a tenant setting that nothing reads yet. | `v0-12-payment.md`. |
| **BIL-OPEN-16** | **The outstanding-balance completion guard is deliberately not registered (BIL-F-17).** A visit is blocked while its invoice is unissued, which billing can clear. It is **not** blocked on an unpaid balance, because nothing can pay an invoice yet and the guard would make every visit with a charge impossible to finish — the same trap `RX` avoided with dispensing. | `v0-12-payment.md` registers it, together with the administrator's "complete with outstanding" override that BIL-F-17 also asks for. |
| **BIL-OPEN-11** | **Void checks a column, not a ledger (BIL-R-07).** It refuses when `amount_paid` is above zero, which is correct and currently vacuous because nothing ever sets it. | `v0-12-payment.md` makes it a real check against payment rows. |
| **BIL-OPEN-12** | **A document fee never appears (BIL-F-02).** Consultations, medicines and procedures all bill themselves. A medical certificate or a medical report would too, and neither exists. | `v0-13-documents.md` calls the same charge registry. |
