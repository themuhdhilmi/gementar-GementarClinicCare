...
| **CON-OPEN-05** | **A signed record still cannot be printed (CON-F-19).** `v0-13-documents.md` is built, and it deliberately left this one out: the certificate, referral, letter, lab request and prescription printouts all exist, but "print the whole note" needs somebody to decide how much of a note an outsider should see, and nobody has. The machinery — letterhead, numbering, storage, hashing, reprint — is all there and waiting. | A template and a route, once that decision is made. Tracked as `DOC-OPEN-11`. |
