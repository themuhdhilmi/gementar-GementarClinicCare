...
| # | Item | Done when |
|---|---|---|
| **ENC-OPEN-04** | **Nothing registers a completion guard.** `EncounterCompletionRegistry` is empty, so a visit can always be finished. Correct today, and wrong the moment prescribing exists. ENC-T-06 plants its own check to prove the guard works. | `CON` registers "consultation unsigned", `RX` registers undispensed items and `BIL` registers an outstanding balance, each with its own test. Lands with `v0-06`, `v0-07` and `v0-11`. |
