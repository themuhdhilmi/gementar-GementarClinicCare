...
| **ENC-OPEN-05** | **The eight queue settings have never been discussed with the clinic.** Triage policy, payment order, the two completion guards, display privacy, the amber and red thresholds, and how many calls before a no-show is suggested. Every default is a guess. **The first four are now shown as the flow they produce** — *"What a visit goes through"* on the Clinic screen — which makes them a conversation the owner can have by looking rather than by reading a dropdown. The other four are still a form. | Read through with whoever runs the clinic, and changed or confirmed on the settings screen. A board that is entirely red because the thresholds are wrong for this clinic is a board nobody looks at. |
...
| **ENC-OPEN-18** | **The board has no procedures tab.** `PROCEDURE_WAITING` is a real status the consultation routes into, `procedure` is a real station with a call target, and the board's tabs are reception, triage, doctor, and then the counter — no procedures. The work is done from the separate **Procedures** screen instead, which works and means the one board the clinic runs its day on does not show everybody who is waiting. | Add it to the board shape the server computes, beside triage. It is a one-line change and the reason it is not done is that nobody has said whether a small clinic wants a fourth tab or would rather keep using the Procedures screen. |
| **ENC-OPEN-17** | **The flow view is clinic-wide only.** *"What a visit goes through"* on the Clinic screen draws the route and edits the four settings that shape it — whether triage happens, which way round pharmacy and payment go, and whether either blocks a visit from closing. **A branch overriding one of those still does it through the ordinary settings form**, with a tick box per field, and sees no diagram. That is right for now: the override UI is tri-state (inherit / override / value) and folding that into a flow chain without making it confusing is a real design problem, not a morning's work. | Either the flow view learns the tri-state, or the branch screen links to it saying which fields this branch has overridden. The second is much cheaper and is probably enough. |
