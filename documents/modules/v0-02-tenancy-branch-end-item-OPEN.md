...
| ~~**TEN-OPEN-16**~~ | ~~**Stock does not block deactivation yet.**~~ **Closed 2026-09-21.** `INV` registers a non-zero-stock check: a branch with batches on its shelves cannot be closed, because closing it would strand the stock where nothing shows it. | Done. |
...
| **TEN-OPEN-08** | ~~**The letterhead is stored and never printed.**~~ **Mostly closed by `v0-13-documents.md`.** Header text, footer text and the logo are now rendered at the top and foot of every A4 document — certificate, referral, letter, prescription, invoice — with the logo inlined as a data URI so a stored document still renders years later without reaching back for a branch that may have closed. Proved in `documents.e2e-spec.ts`. **What remains is the half only paper answers:** nobody has looked at a printed page, so the logo's size and the header's proportions are a guess. | Somebody prints one and looks at it. That is `DOC-OPEN-01`, the printing spike, which is where every remaining paper question now lives. |
