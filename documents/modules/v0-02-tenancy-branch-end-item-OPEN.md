...
| # | Item | Done when |
|---|---|---|
| **TEN-OPEN-01** | **Nothing registers a branch deactivation check (TEN-F-07).** `BranchDeactivationRegistry` is empty, so deactivating a branch is always allowed. That is correct while no clinical or stock module exists, and wrong the moment one does: a branch with medicine on its shelves must not be closable. | `ENC` registers open encounters and `INV` registers non-zero stock, each with its own test, and TEN-T-09 has been rewritten to use one of the real checks rather than the one it plants itself. Lands with `v0-04-encounter-queue.md` and `v0-09-inventory.md`. |
