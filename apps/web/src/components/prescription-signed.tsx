...
export function SignedPrescription({
  ...
  const load = useMemo(
    () => async () => {
      setView(await api<PrescriptionView>(`/consultations/${consultationId}/prescription`));
    },
    [consultationId],
  );
  ...
  for (const item of view.items) {
    if (item.supersedesId) {
      const older = view.items.find((candidate) => candidate.id === item.supersedesId);
    ...
    <Card title="Prescription" description={statusLine(view)}>
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="font-medium">
                    {item.displayName}
                    {item.isControlled && (
                      <span className="ml-2 text-xs font-medium text-danger">Controlled</span>
                    )}
                    {item.version > 1 && (
                      <span className="ml-2 text-xs text-muted">version {item.version}</span>
                    )}
                  </p>
                  <p className="text-xs text-muted">
                    {item.doseValue} {item.doseUnit} · {item.route} · {item.frequencyCode} ·{' '}
                    {item.quantity} {item.quantityUnit} · {statusWord(item)}
                  </p>
                  <p className="mt-0.5 text-xs italic text-muted">{item.labelText}</p>
                  {item.overrideReason && (
                    <p className="mt-0.5 text-xs text-warning">
                      Prescribed over a warning: &ldquo;{item.overrideReason}&rdquo;
                    </p>
                  )}
                  {item.cancelledReason && (
                    <p className="mt-0.5 text-xs text-muted">{item.cancelledReason}</p>
                  )}
                  {older && (
                    <p className="mt-0.5 text-xs text-muted line-through">
                      was {older.doseValue} {older.doseUnit} · {older.frequencyCode} ·{' '}
                      {older.quantity} {older.quantityUnit}
                    </p>
                  )}
                </div>
                {canAmend && item.status !== 'CANCELLED' && item.status !== 'SUPERSEDED' && (
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setError(
                            caught instanceof ApiError ? caught.message : 'Could not cancel it.',
                          );
                        }
                      }}
                    >
      ...
      >
        <p className="text-sm text-muted">
          The original stays exactly as it was. This creates a new version of the item, and the
          pharmacy sees that it changed.
        </p>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setError(caught instanceof ApiError ? caught.message : 'Could not amend it.');
              } finally {
                setBusy(false);
              }
            }}
