...
export default function DisplayPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/v1/display/${token}`, { cache: 'no-store' });
      if (!response.ok) {
        setError(
          response.status === 404
  ...
  return (
    <main className="min-h-screen bg-black px-8 py-6 text-white">
      ...
      <section className="mt-8">
        <h2 className="text-xl uppercase tracking-widest text-white/50">Now serving</h2>
        {view.nowServing.length === 0 ? (
          ...
          <ul className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {view.nowServing.map((row) => (
              <li
                key={row.queueNo}
                className="rounded-2xl bg-white/10 px-6 py-5 ring-1 ring-white/20"
              >
                {/* 60px+ numerals: readable from the far side of the room. */}
                <p className="text-7xl font-bold tabular-nums leading-none">{row.queueNo}</p>
                {row.label && <p className="mt-2 truncate text-2xl text-white/80">{row.label}</p>}
                <p className="mt-1 text-xl uppercase tracking-wide text-white/50">{row.where}</p>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[2fr_1fr]">
        <div>
          ...
          <ul className="mt-4 flex flex-wrap gap-3">
            ...
            {view.waiting.length === 0 && <li className="text-3xl text-white/40">Nobody waiting</li>}
          </ul>
        </div>

        <div>
          <h2 className="text-xl uppercase tracking-widest text-white/50">Just called</h2>
          <ul className="mt-4 flex flex-col gap-2">
            {view.recentlyCalled.map((row) => (
              <li key={`${row.queueNo}-${row.at}`} className="flex items-baseline gap-3 text-2xl">
                <span className="font-semibold tabular-nums">{row.queueNo}</span>
