...
export default function PharmacyPage() {
  ...
  async function openFor(encounterId: string) {
    ...
    try {
      setSession(
        await api<DispenseSession>(`/encounters/${encounterId}/dispense`, {
          method: "POST",
        }),
      );
    } catch (caught) {
  ...
  if (session) {
    return (
      <DispenseSessionView
        session={session}
        onReload={async () =>
          setSession(await api<DispenseSession>(`/dispenses/${session.id}`))
        }
        onClose={async () => {
          setSession(null);
          await refresh();
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        title="Pharmacy"
        description="Prescriptions arrive here the moment a doctor signs the note. Open one to dispense against it."
        actions={
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <Stat label="Waiting" value={rows.length} hint="patients" />
          <Stat label="Items to dispense" value={waiting} />
          <Stat
            label="Longest wait"
            value={`${Math.max(...rows.map((row) => row.waitingMinutes))} min`}
          />
        </div>
      )}
          ...
          {rows.map((row) => (
            <li key={row.encounterId}>
              <div className="flex items-center justify-between gap-4 rounded-xl border border-line bg-surface px-4 py-3 shadow-e1 transition-shadow hover:shadow-e2">
                <div className="min-w-0">
                  ...
                  {(row.hasControlled ||
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      {row.hasControlled && (
                        <Chip tone="danger">Controlled</Chip>
                      )}
                      {row.amended && (
                        <Chip tone="warning">Changed by the doctor</Chip>
                      )}
                      {row.status === "DISPENSING" && (
                ...
                <Button onClick={() => void openFor(row.encounterId)}>
                  Open
                </Button>
              </div>
            </li>
...
function DispenseSessionView({
  ...
  /** Returns whether it worked, so a caller can keep a dialog open. */
  ...
  return (
    <div className="flex flex-col gap-4">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted">
            {session.patient?.mrn} ·{" "}
            <Link
              href={`/encounters/${session.encounterId}`}
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {/* ENC-F-24: the dispenser reads the dose here, so this is
              where the query has to be raisable. Sending them to the
              visit screen to find the button is how it stops being
              used. */}
          <Button variant="quiet" onClick={() => setQuerying(true)}>
            ← Query with the doctor
          </Button>
      ...
      {severe.length > 0 && (
        <Alert tone="danger" title="Allergies">
          {severe
            .map(
              (a) =>
                `${a.substance} (${a.severity?.toLowerCase().replace("_", " ")})`,
            )
            .join(", ")}
        </Alert>
      )}
      ...
      <Modal open={label !== null} title="Label" onClose={() => setLabel(null)}>
        {label && (
          <div className="rounded-md border border-line p-3 text-sm">
            ...
            <p className="text-xs text-muted">
              {label.patientMrn} ·{" "}
              {new Date(label.dispensedAt).toLocaleDateString()}
            </p>
            ...
            <p className="mt-2 text-xs text-muted">
              {label.batches
                .map(
                  (b) =>
                    `Lot ${b.batchNo}${b.expiry ? ` exp ${b.expiry.slice(0, 7)}` : ""}`,
                )
                .join(" · ")}
            </p>
      ...
      {/* ENC-F-24. The patient goes back into the doctor's line with the
          place they had before, and the doctor reads the query on the
          visit. Nothing already handed over is unwound: what was
          dispensed stays dispensed and stays on the record. */}
      <Modal
        open={querying}
        title="Send back to the doctor"
        description="For a dose that cannot be right, an interaction, or nothing on the shelf."
        onClose={() => setQuerying(false)}
      >
        <p className="text-sm text-muted">
          The doctor sees this on the visit, and so does the audit trail.
          Anything already dispensed stays dispensed.
        </p>
        <div className="mt-3">
          <Field label="What is the query?">
            <Input
              value={query}
              autoFocus
              placeholder="Amoxicillin 500mg three times a day for a 9kg child — please confirm"
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={async () => {
              const ok = await act(() =>
                api(`/encounters/${session.encounterId}/transition`, {
                  method: 'POST',
                  body: { to: 'DOCTOR_WAITING', note: query.trim() },
                }),
              );
              // A refusal — no permission, or the visit moved on under
              // them — has to stay on screen with the query still typed.
              if (!ok) return;
              setQuerying(false);
              setQuery('');
              await onClose();
...
function ItemCard({
  ...
  busy: boolean;
  onAct: (run: () => Promise<unknown>) => Promise<boolean>;
  ...
  return (
    <li className="rounded-lg border border-line bg-surface p-4">
        ...
        <div>
          <p className="font-medium">
            ...
            {item.amendedSinceOpen && (
              <span className="ml-2 align-middle text-xs font-medium text-warning">
                Changed
              </span>
            )}
            {item.newSinceOpen && (
      ...
      {item.isExternal && (
        <Alert tone="info">
          Written for the patient to have filled elsewhere. Nothing here to hand
          over.
        </Alert>
      )}
      ...
      {!done && !settled && !item.isExternal && (
        <>
          {item.suggestion.length > 0 ? (
            ...
            <Alert tone="warning">
              Nothing usable on the shelf. Substitute it, or mark it for the
              patient to obtain elsewhere.
            </Alert>
          )}
            ...
            <Button
              ...
              onClick={() => {
                const why = window.prompt(
                  "Why is the patient not taking this?",
                );
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
          ...
          <Button
            ...
            onClick={() =>
              void onAct(async () => {
                onLabel(
                  await api<DispenseLabel>(`/dispense-items/${done.id}/label`, {
                    method: "POST",
                  }),
                );
              })
...
function SubstituteModal({
  ...
  return (
    <Modal open title={`Instead of ${item.displayName}`} onClose={onClose}>
      {error && <Alert tone="danger">{error}</Alert>}
      <p className="text-sm text-muted">
        Another brand of {item.genericName} is routine. A different medicine is
        a prescribing decision, and will be refused unless you are allowed to
        make it.
      </p>
          ...
          onClick={async () => {
            ...
            try {
              await api(
                ...
                {
                  method: "POST",
                  body: { productId: chosen.id, reason: reason.trim() },
                },
              );
              await onDone();
            } catch (caught) {
